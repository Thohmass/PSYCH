...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    if (postParams.parentPostId != null && postParams.parentPostId != "") {
      const parentPost =
        await forumPostsCollection.doc(postParams.parentPostId).get();
      if (!parentPost) {
        res.status(404).json({
          message: "Nepodarilo sa identifikovať pôvodný príspevok.",
        });
        return;
      }
    }
    if (postParams.rootPostId != null && postParams.rootPostId != "") {
      const rootPost =
        await forumPostsCollection.doc(postParams.rootPostId).get();
      if (!rootPost) {
        res.status(404).json({
          message: "Nepodarilo sa identifikovať koreňový príspevok.",
        });
        return;
      }
    }
    if (postParams.authorUserId != null && postParams.authorUserId != "") {
      const authorRef =
        forumPostsCollection.doc(postParams.authorUserId).get();
      if (!authorRef) {
        res.status(404).json({
          message: "Nepodarilo sa identifikovať autora nového príspevku.",
        });
        return;
      }
    }
    ...
    res.status(201).send({
      message: "Nový príspevok bol úspešne vytvorený na fóre.",
      id: postRef.id,
      post: postParams,
    });
  } catch (error: unknown) {
    ...
    res.status(500).json({
      message: "Nepodarilo sa vytvoriť nový práspevok do fóra.",
    });
  }
};
...
export const getRootPosts = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      message: "Nepodarilo sa načítať príspevky fóra.",
    });
  }
};
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    console.error(`Chyba pri získavaní detailu príspevku fóra s ID ${
      req.params.postId
    }:`, error);
    res.status(500).json({
      message: "Nepodarilo sa načítať detail príspevku fóra.",
    });
  }
};
